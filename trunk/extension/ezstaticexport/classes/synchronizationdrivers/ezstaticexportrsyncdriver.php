...
class eZStaticExportRsyncDriver extends eZStaticExportSyncAbstractDriver
{
    ...
    function startSync()
    {
        ...
        if ( $return != 0 )
        {
            eZStaticExportLogger::log("An error occured syncing to ".$this->ConnectionInfos['host']." using " . $this->DriverName, eZStaticExportLogger::LOG_TYPE_WARNING );
            echo implode( "\n", $output );
            return false;
        }
        else
        {
            ...
            eZLog::write("Rsync Output : \n".implode("\n", $output), 'staticexport-rsync.log');
            return true;
        }
    }
