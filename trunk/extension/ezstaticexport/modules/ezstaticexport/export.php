...
if ( $Module->isCurrentAction( 'DoExport' ) )
{
    ...
    if ( $Module->actionParameter( 'Schedule' ) == 'scheduled' )
    {
        ...
        else
        {
            ...
            if ($Module->actionParameter('Type') == 'subtree')
            {
	            ...
	            $row['total']  = eZContentObjectTreeNode::subtreeCountByNodeID( $aSubtreeParams, $topExportNode->attribute( 'node_id' ) );
            }
            else
            {
            	$row['total'] = 1;
    ...
    else
    {
        ...
        if ( $valid )
        {
            $user = eZUser::currentUser();
            $row['user_id'] = $user->attribute( 'contentobject_id' );
            $row['status']          = eZStaticExport::STATUS_PENDING;
